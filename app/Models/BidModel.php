...
public function getdataforwin($id){
 
    $builder = $this->db->table('bid_table'); // Select the 'bid_table' and create a query builder

$builder->select('bid_table.*, g_service.*, gn_type.*, user_log.user_id, user_log.user_name'); // Select the columns you need from multiple tables

$builder->join('user_log', 'bid_table.user_id = user_log.user_id', 'inner'); // INNER JOIN with 'user_log' table
$builder->join('g_service', 'g_service.g_id = bid_table.g_id', 'inner'); // INNER JOIN with 'g_service' table
$builder->join('gn_type', 'gn_type.gt_id = bid_table.gt_id', 'inner'); // INNER JOIN with 'gn_type' table

$builder->where('bid_table.b_id', $id);
$query = $builder->get();
        $result = $query->getResult();
        ...
        if (empty($result)) {
            throw new Exception('No data found in for winer.');
        }
